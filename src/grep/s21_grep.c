    ...
    for (int j = 0; j < count_files; j++) {
        ...
        if ((file = fopen(files[j], "r")) == NULL) {
            ...
            while (getline(&string, &size_file, file) != -1) {
                searching(&flags, count_regex, count_files, &gex, &match,
...
void parser(int argc, char *argv[], Flags *flags, char regex[100][1000], char files[100][1000],
    ...
    for (int i = 0; i < count_regex; i++) {
        ...
        if (flags->v) {
            ...
            if (regexec(gex, string, 1, match, 0) != 0) {
                if (flags->c && flags->l) { *count_for_c = 1; *count_for_l = 1; regfree(gex); break; }
                if (flags->c) { *count_for_c += 1; regfree(gex); break; }
                if (flags->l) { *count_for_l += 1; regfree(gex); break; }
